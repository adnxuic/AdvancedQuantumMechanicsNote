\chapter{Evaluation of the Fresnel Integral}
\label{appendix:fresnel}
...
\section{Derivation (Using Contour Integration)}
\textbf{Step 1: Define the Contour}
...
\textbf{Step 2: Apply Cauchy's Integral Theorem}
...
\textbf{Step 3: Evaluate the Integral on Each Path}

\textbf{1. Integral along Path $C_1$ (The part we want to find)}
...
\textbf{2. Integral along Path $C_2$ (Show it vanishes as $R \to \infty$)}
...
\begin{equation}
    ...
    &= R \left.\left[ \frac{-\mpi}{4R^2} \exp(-4R^2\theta/\mpi) \right] \right|_0^{\mpi/4} \\
...
\textbf{3. Integral along Path $C_3$ (Connection to the Gaussian Integral)}
...
\textbf{Step 5: Calculate the Final Integral}
