...
&= -\frac{1}{2} \iint_{\partial A_r} \mathrm{d}\Omega \\
&= -\frac{1}{2} A_r \quad (\text{Because the radius is } 1 \text{ and } A_r = R^2 \Omega).
\end{aligned}
\end{equation}
where $r$ is the path $|n(t)\rangle$ from $|n(t_i)\rangle=|n_i\rangle$ to $|n(t_f)\rangle=|n_f\rangle$
and $A_r$ is area enclosed by three points on the sphere ($|\uparrow\rangle, |n_i\rangle, |n_f\rangle$). Obviously, $A_r$ depends on where the North pole is defined, which is kind of "gauge-dependence"
