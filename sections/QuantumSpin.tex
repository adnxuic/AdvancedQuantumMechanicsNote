...
\begin{itemize}
    \item This discretely parameterized complete set
    is not convenient for constructing the path-integral formalism for quantum spins.
    \item The $SU(2)$ spin symmetry is either explicitly broken or not manifest.
\end{itemize}
...
Next, let us prove that $\braket{-\bm{n}}{\bm{n}} = 0$. The antipodal point $-\bm{n}$ corresponds to the angles $(\theta', \phi') = (\mpi - \theta, \phi + \mpi)$.
...
\textbf{Distinction between $\ket{-\bm{n}}$ and $-\ket{\bm{n}}$}
...
First, consider the case where $\hat{H}$ describes an isolated spin.
...
We now insert the resolution of the identity operator. Let us label our
...
Let us represent the state at time $t_l$ as a small deviation
...
Because $\theta$ and $\varphi$ depend on $t$, we can express $\theta$ as a function of $\varphi$.

We can rewrite the function $1-\cos\theta(\varphi)$ as an integral:
\begin{equation}
1-\cos\theta(\varphi) = \int_0^{\theta(\varphi)} \mathrm{d}\theta \cdot \sin\theta
\end{equation}

So the action can be written as an area integral:
\begin{equation}
    \colorboxed{
    \begin{aligned}
...
and $A_{\gamma}$ is the area enclosed by three points on the sphere ($|\uparrow\rangle, |n_i\rangle, |n_f\rangle$). Obviously, $A_{\gamma}$ depends on where the North pole is defined, which is a kind of "gauge dependence".
If we multiply $|n(t)\rangle$ by an overall phase:
\begin{equation}
|n(t)\rangle \longrightarrow \mathrm{e}^{\mathrm{i}\delta} |n(t)\rangle
\end{equation}

$S[n(t)]$ acquires an additional term:
\begin{equation}
\begin{aligned}
...
If we choose a different gauge, $\Delta\delta$ changes,
so $S[n(t)]$ is gauge-dependent.

\input{Tikz/IntegralPath.tex}

However, for a closed path ($n_f=n_i$), $S[n(t)]$ is
gauge-independent:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
The reason is as follows: to ensure that the transformed
basis is single-valued, we require
\begin{equation}
...
\end{equation}
So for a closed path, $S[n(t)]$ (more precisely, the geometric phase factor)
\begin{equation}
...
\end{equation}
remains unchanged; equivalently, the geometric phase $S$ is
invariant modulo $2\pi$.
...
where $\bm{n}$ can be regarded as the monopole field strength with unit charge.

\input{Tikz/monopole.tex}

Because $\bm{n}$ lies on the surface of the sphere,
it can be a function of two parameters $t$ and $k$,
...
The Wess-Zumino term has another commonly used expression:
...
Consider a spin system on a $d$-dimensional lattice, called the Heisenberg model (figure \ref{fig:HeisenbergModel}):
\begin{equation}
    \colorboxed{
    \hat{H} = J \cdot \sum_{\langle i,j \rangle} \hat{\bm{S}}_i \cdot \hat{\bm{S}}_j
    }
\end{equation}
where $J$ is the exchange coupling and $\hat{\bm{S}}$ are quantum-spin operators.
...
\end{equation}
where
$|\bm{n}_0 \rangle = \bigotimes_{i=1} |\bm{n}_0^i \rangle$ is the tensor product of all states on the lattice.
...
\end{equation}
In this version, the propagator $K$ can be written as (for a closed path):
\begin{equation}
...
Because the diagonal element of the partition function and the propagator have the same mathematical form, we can use the same method to evaluate the diagonal elements.

Let $\Delta \tau = \frac{\beta}{N}$ ($N$ is the number of slices),
which means that $\beta$ plays the same role as $\tau$ in the diagonal elements, analogous to $\tau$ in the propagator
$(\tau_i \to \tau_f \Longrightarrow 0 \to \beta)$.

But in a many-spin system, the identity operator for the entire system is the tensor product of the identities for each site:
\begin{equation}
...
\end{equation}
So the diagonal element can be written in a form analogous to the propagator:
\begin{equation}
...
Because the diagonal element corresponds to a closed path (i.e., $\langle \bm{n}_0 | \mathrm{e}^{-\beta \hat{H}} | \bm{n}_0 \rangle$, not $\langle \bm{n}_i | \mathrm{e}^{-\beta \hat{H}} | \bm{n}_j \rangle$ for $i \neq j$), $S_E$ can be written as:
\begin{equation}
...
So $\{ \sum_i S_{WZ} [\bm{n}_i] \}$ and $\{ \int_0^\beta \mathrm{d}t \sum_{\langle i,j \rangle} J \cdot \bm{n}_i \cdot \bm{n}_j \}$ can be regarded as phase terms in the action ($\mathrm{e}^{\frac{\mathrm{i}}{\hbar} \theta}$). Therefore, $S$ and $\frac{S^2}{\hbar}$ play the role of $\hbar$ in the simple action.
Here, if $S$ is very large, we call it the large-$S$ limit ($\hbar \ll 1$, so $\frac{1}{\hbar} \approx S \gg 1$).
This is a semiclassical limit.

\section{Quantum Ferromagnetism}

Let us consider a ferromagnetic system with $J = -|J|$. The action is:
\begin{equation}
S_M = S \sum_i S_{WZ} [\bm{n}_i] + |J|S^2 \sum_{\langle i,j \rangle} \int_0^\beta \mathrm{d}t \bm{n}_i \cdot \bm{n}_j
\end{equation}

Since $|\bm{n}|^2 = 1$, we can rewrite the term $\bm{n}_i \cdot \bm{n}_j$ as:
\begin{equation}
\begin{aligned}
...
The second term of $S_e$ is a constant (a global phase), so we can drop it and $S_M$ can be written as:
\begin{equation}
...
\end{equation}
We first need the variation of the Wess-Zumino term $\delta S_{\mathrm{WZ}}$. 
Geometrically, 
...
We take the cross product with $\bm{n}$ on both sides of the equation:
\begin{equation}
\begin{aligned}
...
Therefore, the variation of the Wess-Zumino part of the action is:
\begin{equation}
...
\end{equation}
Now, substitute $\lambda$ back into the equation:
\begin{equation}
...
From this equation, we see that the spins undergo precessional motion with an angular velocity $\bm{\Omega}$ given by:
\begin{equation}
\bm{\Omega} = -|J|S a_0^2 \nabla^2 \bm{n}.
\end{equation}
...
\sigma^2 + \bm{\pi}^2 = 1
\end{equation}
If the system is ordered, $\sigma \sim 1$ and $|\bm{\pi}|$ is very small, which means that $\bm{\pi}$ is a small fluctuation around the direction of $\sigma$.
...
\end{equation}
Thus, the linearized Landau-Lifshitz equations are:
\begin{equation}
\begin{cases}
...
We look for plane-wave solutions (spin waves) of the form:
\begin{equation}
\pi_j(\bm{x}, t) = \tilde{\pi}_j e^{\mathrm{i}(\bm{p} \cdot \bm{x} - \omega t)}
\end{equation}
Taking derivatives of this ansatz converts differential operators into algebraic variables:
\begin{equation}
\begin{cases}
...
\end{cases}
\end{equation}
Let $A = |J|S a_0^2 p^2$; the system becomes:
\begin{equation}
\begin{cases}
...
\section{Quantum Antiferromagnetism in One Dimension}

Consider a spin chain with an even number of sites $N$; we can write down its action ($J=|J|$):
\begin{equation}
    \label{eq:action_afm}
...
For an antiferromagnet, we can split $\bm{n}$ into a slowly varying piece $\bm{m}$ plus a small, rapidly varying part $a_0 \bm{l}$, so:
\begin{equation}
...
\end{equation}
Now we substitute the expression for $\bm{l}$ found above back into $\mathcal{L}_{\bm{l}}$:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Finally, we can combine the new term with the original terms that do not depend on $\bm{l}$:
\begin{equation}
    ...
    \end{aligned}
    \end{equation}
where $g$ and $v_s$ are the coupling constant and the spin-wave velocity, respectively:
\begin{equation}
\begin{cases}
g = \frac{2}{S} \\
v_s = 2 a_0 |J| S
