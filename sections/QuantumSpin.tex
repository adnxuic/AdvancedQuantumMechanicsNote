...
It means that:
\begin{equation}
S_{\mathrm{WZ}} = -\iint_{Ar} \mathrm{d}\Omega = - \iint_{Ar} \bm{n} \cdot \mathrm{d}\bm{S}
\end{equation}

where $\bm{n}$ can regrad as the field strength whose charge is 1. 

\input{Tikz/monopole.tex}

Because $\bm{n}$ locate in the sphere, it can be the function of two parameter $t$ and $k$, which form an orthonormal basis on the sphere. So $\mathrm{d}\bm{S}$ can be written as:

\begin{equation}
\begin{aligned}
\mathrm{d}\bm{S} &= \mathrm{d}\bm{n}(t, k) \times \mathrm{d}\bm{n}(t, k+\mathrm{d}k) \\
&= (\frac{\partial \bm{n}}{\partial t} \cdot \mathrm{d}t) \times (\frac{\partial \bm{n}}{\partial k} \cdot \mathrm{d}k) \\
&= (\frac{\partial \bm{n}}{\partial t} \times \frac{\partial \bm{n}}{\partial k}) \mathrm{d}t \cdot \mathrm{d}k.
\end{aligned}
\end{equation}
...
\begin{equation}
S_{\mathrm{WZ}} = -\int_{0}^{1} \mathrm{d}k \int_{t_i}^{t_f} \mathrm{d}t \cdot \bm{n}(t, k) \cdot \left[ \frac{\partial \bm{n}}{\partial t} \times \frac{\partial \bm{n}}{\partial k} \right]
\end{equation}

where $k$ is an auxiliary parameter, in order to extend $\bm{n}(t)$ to $\bm{n}(t, k)$ with:
\begin{equation}
\begin{cases}
\bm{n}(t, 0) = \bm{n}(t) \\
\bm{n}(t, 1) = \bm{n}_0 (\text{North Pole}) \\
\bm{n}(t_i, k) = \bm{n}(t_f, k) \ (\text{Closed Path})
\end{cases}
\end{equation}
...
define the Gauge Potential $\bm{\mathcal{A}}$:
\begin{equation}
...
we apply a local gauge transformation where the state $z$ picks up a position-dependent phase $\phi(\bm{x})$.
\begin{equation}
