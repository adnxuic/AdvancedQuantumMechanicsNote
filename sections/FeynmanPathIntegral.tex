...
\begin{equation}
\mathrm{i}G = K(x_f, t_f; x_0, t_0) = \int \mathcal{D}[x(t)] \mathrm{e}^{\frac{\mathrm{i}}{\hbar} S[x(t)]}
\end{equation}
where:
\begin{itemize}
...
\end{equation}
where $a$ is a small parameter. and the function $f(x)$ is real-valued and regular.

Our objective is to understant the physical picture of the startiomong phuse approximation for the propagutor ph path integral through this one dimensional integral stortionary phase approximation.
...
In order to compute the integral $I$, what we really need to do is to find "dominant contribution" to $I$.
...
Within this small domain, we may linearize $\Theta(x)$ by:
\begin{equation}
\begin{aligned}
...
\begin{equation}
\begin{aligned}
I_{(x_0)}^{\Delta x} &\sim \mathrm{e}^{\mathrm{i}\Theta_0} \frac{a}{\mathrm{i}f'_0} \cdot \left. \mathrm{e}^{\frac{\mathrm{i}f'_0}{a}(x-x_0)} \right|_{x_0-\frac{\Delta x}{2}}^{x_0+\frac{\Delta x}{2}} \\
...
Because $a$ is a small parameter very small but nonzero parameter (in quantum mechanics, it corresponds to $\hbar$ being a very small but nonzero number),
...
\end{equation}
Physically, the result means that all U(1) phase in the vicinity of $x_0$ are completely the same. It's a perfect phase-coherence. The superposition of constant phases leads to linearly enhanced amplitude "$\Delta x$".
...
\end{equation}
it can be regarded as path integral version of $\int \mathrm{d}x \mathrm{e}^{\frac{\mathrm{i}}{a}f(x)}$. $f(x)$ is replaced by Classical action $S_c$ when $\delta S_c=0$. Liking the previous section, we consider. the quadratic approximation in the vicinity of classical. path:
\begin{equation}
\begin{aligned}
...
Now, we consider can decompose the path near classical path into the classical path $x_c(t)$ plus a quantum fluctuation $y(t)$ around it:
\begin{equation}
x(t) = x_c(t) + y(t)
\end{equation}
...
\begin{equation}
\begin{aligned}
K &\approx \int \mathcal{D}[x(t)] \exp\left\{\frac{\mathrm{i}}{\hbar}\left[S_c + \frac{1}{2}\delta^2 S\right]\right\} \\
...
\end{aligned}
\end{equation}
Now, we need to change the integration variable from an integral over all paths $x(t)$ to an integral over all fluctuations $y(t)$. Since $x_c(t)$ is a fixed. path, the path measure:
\begin{equation}
\mathcal{D}[x(t)] = \mathcal{D}[x_c(t)+y(t)] = \mathcal{D}[y(t)]
\end{equation}
and $S_c$ is a constant, it can be factored out of integral:
\begin{equation}
K \approx \mathrm{e}^{\frac{\mathrm{i}}{\hbar}S_c} \cdot \int \mathcal{D}[y(t)] \cdot \mathrm{e}^{\frac{\mathrm{i}}{2\hbar} \int \mathrm{d}t \left(\frac{1}{2}m\dot{y}^2 - \frac{1}{2}V''(x_c)y^2\right)}
\end{equation}
\begin{equation}
...
\begin{example}
    ...
    \begin{equation}
    \begin{aligned}
    K &= J \cdot \left(\frac{2\pi\mathrm{i}\hbar\Delta t}{m}\right)^{N/2} \cdot \left(\frac{m}{2\pi\mathrm{i}N\Delta t}\right)^{1/2} \cdot \mathrm{e}^{\frac{\mathrm{i}S_c}{\hbar}} \\
    ...
    &= \left(\frac{m}{2\pi\mathrm{i}(t_f-t_0)}\right)^{1/2} \cdot \mathrm{e}^{\frac{\mathrm{i}S_c}{\hbar}}
    \end{aligned}
    \end{equation}
