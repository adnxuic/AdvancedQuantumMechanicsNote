...
\begin{enumerate}
    \item Schrödinger's wave function (operator form):
    \begin{equation}
        \hat{H}|\psi\rangle = \mathrm{i}\hbar\frac{\partial}{\partial t}|\psi\rangle
    \end{equation}
    \item Feynman's path integral (c-number form):
    \begin{equation}
        \mathrm{i}G(\text{Green's function}) \propto \int \mathcal{D}(x, t) \mathrm{e}^{\mathrm{i}\int \mathcal{L} \mathrm{d}t}
    \end{equation}
\end{enumerate}

There are many advantages of the Feynman path integral:
\begin{enumerate}
    \item It makes the double-slit experiment more understandable.
    \item The classical limit \enquote{$\hbar \to 0$} is \enquote{tractable}:
    quantum $\xrightarrow{\hbar \to 0}$ classical.
    \item It provides a semiclassical picture of quantum mechanics.
    \item \enquote{Quantum fluctuations} are more \enquote{understandable}.
    \item A natural route to the low-energy effective theory of quantum many-body systems.
    \item A natural language for describing topological properties of quantum many-body systems.
\end{enumerate}

However, practical calculations in the path-integral representation
of simple quantum-mechanical problems may be notoriously
difficult and lengthy.

\section{Propagators}
Consider a quantum particle confined in a one-dimensional space:
\begin{equation}
...
Assuming the Hamiltonian $H$ does not explicitly depend on
...
Moreover, $\hat{U}(t, t_0)$ is unitary:
\begin{equation}
...
We can define a propagator (Green's function) of the quantum system using these matrix elements:
\begin{equation}
\mathrm{i}G(x, t; x_0, t_0) = U(x, t; x_0, t_0)
\end{equation}

Using the matrix elements, $\psi(x, t)$ can be rewritten as:
\begin{equation}
\begin{aligned}
...
The propagator also satisfies the Schrödinger equation:
\begin{equation}
...
\end{equation}
$\delta(x-x_0)$ is the Dirac delta function.
...
\begin{example}
    ...
    Using the Fourier transform: (we use $G(x,t)$ instead of $G(x,t;x_0,t_0)$). We solve the free-particle Green's function by transforming to momentum space:
    \begin{equation}
        \left\{
        \begin{aligned}
    ...
    Alternatively, we can solve this PDE directly from the definition:
    \begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
    \end{equation}
    Using $P = \hbar k$, we obtain the same result as in the Fourier-transform method.
...
When $t > t_1 > t_0$ and $t_1$ is an arbitrarily chosen intermediate time,
we can write:
\begin{equation}
    \begin{aligned}
...
This integral over $x_1$ represents the \enquote{superposition} of all possible \enquote{paths} that connect $x$ and $x_0$.
Next, we try to \enquote{smooth} the path in time directly.
We can insert more time slices between $x$ and $x_0$.
If we insert infinitely many time slices, the path becomes smooth.

\begin{center}
\input{Tikz/SmoothIntegralPathProcess.tex}
\end{center}

First, let us discretize the time domain $[t_0, t]$ into $N$ pieces of equal length $\Delta t = \frac{t-t_0}{N}$:
\begin{equation}
\begin{aligned}
...
With the approximation $V(x_l) \approx V(x_{l-1})$:
\begin{equation}
...
Thus, $\mathrm{i}G(x_l, t_l; x_{l-1}, t_{l-1})$ can be written as:
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
where $H_l$ is the classical Hamiltonian as a function of $p_l$ and $x_l$.
...
So $\mathrm{i}G(x_l, t_l; x_{l-1}, t_{l-1})$ can be written in a Lagrangian form:
\begin{equation}
...
Let $\mathcal{D}_p = \prod_{l=1}^{N} \frac{\mathrm{d}p_l}{2\pi\hbar}$. When $\Delta t \to 0$, this means:
\begin{equation}
\sum_{l=1}^{N} \mathcal{L}_l \Delta t = \int_{t_0}^{t} \mathrm{d}\tau \cdot \mathcal{L}[p(\tau), x(\tau)]
\end{equation}

Finally, we can obtain the propagator from the path integral:
\begin{theorem}
...
\begin{center}
\input{Tikz/SmoothIntegralPath.tex}
\end{center}

\section{Gaussian Integration}

If the functional integration over $p$ is Gaussian, we can integrate out $p$ exactly. For example, $H = \frac{p^2}{2m} + V$,
so $\mathcal{L} = p\dot{x} - H = p\dot{x} - \frac{p^2}{2m} - V(x)$, and we obtain:
...
We have a useful formula for the Gaussian integral (Proof in \ref{appendix:gaussian-integral}):
\begin{equation}
    \colorboxed{
...
\end{equation}
where $\bm{x}, \bm{y}$ are real vectors and $A$ is a real symmetric matrix.
...
Thus, we can integrate out $p$ to obtain:
\begin{equation}
    \begin{aligned}
...
\end{equation}
where $S[x(t)] = \int_{t_0}^{t} \mathrm{d}t \, \mathcal{L}(x, \dot{x})$ is the action
and $\mathcal{L}(x, \dot{x}) = \frac{m\dot{x}^2}{2} - V(x)$ is the Lagrangian.

From the path integral in real spacetime, we can gain some intuition about the physical picture:
\begin{enumerate}[(1)]
    \item Each path is weighted with a $U(1)$ phase factor $e^{\frac{\mathrm{i}}{\hbar}S}$.
    This describes quantum interference between different paths.
    \item Since $\hbar \sim 10^{-34} \, \mathrm{J \cdot s}$, any "small change" in S (we change S to $S+\delta S$)
    can lead to strong destructive interference.
    Therefore, only the paths that satisfy $\delta S=0$ make dominant contributions to the path integral.
    \item Remarkably, $\delta S=0$ is exactly Hamilton's principle in classical mechanics.
    Thus, the classical paths ($\delta S=0$) dominate the path integral in the limit $\hbar \to 0$. In other words,
    as $\hbar \to 0$, contributions from other nearby paths cancel out by interference.
...
\begin{example}
    The Hamiltonian of a free particle is $H = \frac{p^2}{2m}$.\

    Using this Hamiltonian, we can obtain the path integral:
    \begin{equation}
    ...
    \end{equation}
    we should rewrite the exponent $\exp[\frac{\mathrm{i}m}{2\hbar\Delta t} \sum (x_l^2-2x_l x_{l-1} + x_{l-1}^2)]$,
    and define:
    \begin{equation}
    ...
    $\bm{y} = \begin{bmatrix} -x_N \\ 0 \\ \vdots \\ 0 \\ x_0 \end{bmatrix}$ has only two non-zero elements,
    which are located in the first row and the last row, respectively.
...
We have obtained the propagator $\mathrm{i}G$ for a particle to travel from a spacetime point $(x_0, t_0)$ to a final spacetime point $(x_f, t_f)$,
which is given by the Feynman path integral:
\begin{equation}
...
where:
\begin{itemize}
    \item $x(t)$ is the position of the particle at time $t$, representing a possible path.
...
\end{itemize}
This integral is infinite-dimensional and generally very difficult to calculate directly. So we need an effective method to approximate it.
The stationary phase approximation provides a method for approximating it.

\section{One-dimensional stationary-phase approximation}
Consider an integral:
\begin{equation}
I = \int \mathrm{e}^{\mathrm{i} f(x)/a} \mathrm{d}x
\end{equation}
where $a$ is a small parameter and $f(x)$ is a real-valued, smooth function.

Our objective is to understand the physical picture of the stationary-phase approximation for the propagator path integral via this one-dimensional stationary-phase approximation.

Let us further define a new function $\Theta(x)$ as:
\begin{equation}
...
\end{equation}
This integral can be regarded as an interference experiment.

Because each source at $x$ contributes a phase factor $\mathrm{e}^{\mathrm{i}\Theta(x)}$, 
so the total integral $I$ is the result of adding up infinitely many tiny vectors.

In order to compute the integral $I$, 
what we really need to do is to find the "dominant contribution" to $I$.

First, let us pick a point $x_0$ and evaluate the integral near $x_0$.
...
\end{aligned}
\end{equation}
Let $\Theta(x_0)=\Theta_0$ and $f'(x_0)=f'_0$, so:
\begin{equation}
\begin{aligned}
...
\end{equation}
Physically, the result means that all $U(1)$ phases in the vicinity of $x_0$ are completely the same.
It is perfect phase coherence. The superposition of constant phases leads to a linearly enhanced amplitude "$\Delta x$".
...
The smaller $a$ is, the more severe the destructive interference becomes.

In conclusion, if we consider a sufficiently small but nonzero parameter $a$,
it is computationally economical to focus on the contributions from the vicinity of the special points (denoted by a set $\{x_0^i\}$) that satisfy $f'(x_0^i)=0$.

\begin{center}
\input{Tikz/stationary_points.tex}
\end{center}
...
\end{aligned}
\end{equation}
Because the parameter $a$ is a very small number, far away from the stationary point $x_0^i$,
...
Therefore, in the above Gaussian integral near $x_0$, we can extend the integral bounds to infinity:
\begin{equation}
    \colorboxed{
    \begin{aligned}
...
We have discussed that if $\hbar$ tends to zero, then the quantum system will transition to the classical system.
We only need to treat $f(x)$ as $S[x(t)]$ and the parameter $a$ as $\hbar$ to see the reason, based on the discussion in the previous section.
If $\hbar$ is zero, $I^{\Delta x}(x_0)$ equals zero strictly for $x \neq x_0$ and is nonzero only at $x=x_0$.
So we only need to consider the classical path with $\delta S=0$ and do not need to consider the quantum fluctuation near the classical path.
But if $\hbar$ is a very small but nonzero number, we need to consider the quantum fluctuation near the classical path. 
In other words, in classical mechanics,
$\hbar=0$, $I^{\Delta x} \propto \frac{\sin\alpha}{\alpha}$ is equal to zero in $(x_0-\frac{\Delta x}{2}, x_0) \cup (x_0, x_0+\frac{\Delta x}{2})$,
because $\alpha = \frac{f'_0 \Delta x}{2a}$ tends to infinity no matter how small the radius of this deleted neighborhood is.
But in quantum mechanics, $\hbar \sim 10^{-34} \mathrm{J}\cdot\mathrm{s}$, 
$I$ is not equal to zero in a neighborhood whose radius is on the order of $\hbar$.
Therefore, we cannot ignore the contribution from $I$ in this neighborhood.

The Feynman path integral:
\begin{equation}
K(x_f, t_f; x_0, t_0) \propto \int \mathcal{D}[x(t)] \cdot \mathrm{e}^{\frac{\mathrm{i}}{\hbar}S[x(t)]}
\end{equation}
It can be regarded as the path-integral version of $\int \mathrm{d}x \mathrm{e}^{\frac{\mathrm{i}}{a}f(x)}$.
$f(x)$ is replaced by the classical action $S_c$ when $\delta S_c=0$.
Like the previous section, we consider the quadratic approximation in the vicinity of the classical path:
\begin{equation}
\begin{aligned}
...
Now, we can decompose the path near the classical path into the classical path $x_c(t)$ plus a quantum fluctuation $y(t)$ around it:
\begin{equation}
x(t) = x_c(t) + y(t)
\end{equation}
...
\end{cases}
\end{equation}
Let us perform a functional Taylor expansion of the action $S[x(t)]$ around the classical path $x_c(t)$:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
In the semiclassical approximation, we assume the fluctuations $y$ are small, so we neglect terms of $O(y^3)$ and higher. At the same time, $\delta S=0$, so the action $S[x(t)]$:
\begin{equation}
...
\end{equation}
and $S_c$ is a constant, so it can be factored out of the integral:
\begin{equation}
    \colorboxed{
    \begin{aligned}
...
\end{enumerate}
Now let us consider an example: a one-dimensional free particle.
\begin{example}
    For free particle:
    \begin{equation}
    \mathcal{L}(x, \dot{x}) = \frac{1}{2}m\dot{x}^2.
    \end{equation}
    First, we need to calculate the classical action. It is given by the Euler-Lagrange equation:
    \begin{equation}
    ...
    \end{equation}
    Now let us calculate the quantum fluctuation factor:
    \begin{equation}
    \begin{aligned}
    ...
    So we can get the complete propagator of a free particle:
    \begin{equation}
    \begin{aligned}
